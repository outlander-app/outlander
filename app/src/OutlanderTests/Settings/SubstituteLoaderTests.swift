...
class SubstituteLoaderTests: XCTestCase {
    ...
    func test_load() {
        ...
        var sub = context.substitutes.all().first!
        ...
        sub = context.substitutes.all().dropFirst(1).first!
    ...
    func test_save() {
        ...
        loader!.load(context.applicationSettings, context: context)
        loader!.save(context.applicationSettings, subsitutes: context.substitutes.all())
